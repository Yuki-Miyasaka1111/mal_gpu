...
# ディレクトリのパス
extract_all_data_folder = '/workspaces/mal_gpu/data/interim/extract_all_data'

# トークナイザー訓練用のデータを保存するファイル
tokenizer_training_file = '/workspaces/mal_gpu/data/interim/tokenizer_training_data01.csv'
...
with open(tokenizer_training_file, 'w', encoding='utf-8', newline='') as csvfile:
    ...
    # サブディレクトリを含むすべてのJSONファイルを再帰的に処理
    for root, dirs, files in os.walk(extract_all_data_folder):
